          ...
          <b-table
            ...
            <template #empty>
              <div class="text-center">
                No Free Paper Abstracts were found.
              </div>
            </template>
            ...
            <template v-slot:cell(action)="row">
              <button class="btn btn-blue w-25 py-2 corner-22px opensans w-50 corner-pill mb-2" @click="openAbstractModal(row.item.id)">View</button>
<!--              <button v-if="adminCapability.can_delete_abstract"-->
<!--                      class="btn btn-red w-25 py-2 corner-22px opensans w-50 corner-pill" :class='"btn-delete-abstract-" + row.item.id'-->
...
export default {
  ...
  methods: {
    getFreePapers(){
      this.$store.dispatch("abstract/getAllFreePaperAbstracts")
        .then(res => {
  ...
  mounted() {
    this.setNavbarTitle('Free Paper Presentation')
